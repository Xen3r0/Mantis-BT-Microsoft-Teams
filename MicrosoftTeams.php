...
class MicrosoftTeamsPlugin extends MantisPlugin {
    var $skip = false;
    ...
    function register() {
        ...
        $this->author = 'Hamdi Amine';
        $this->contact = 'Amine.hamdi@b2m-it.com';
        $this->url = 'http://b2m-it.com/';
    }
    ...
    function bugnote_add_form($event, $bug_id) {
        ...
        echo '<td colspan="5">';
        echo '<label>';
        echo '<input ', helper_get_tab_index(), ' name="MicrosoftTeams_skip" class="ace" type="checkbox" />';
    ...
    function format_text($bug, $text) {
        $t = string_display_line_links($this->bbcode_to_MicrosoftTeams($text));
    ...
    function notify($msg, $webhook, $channel, $attachment = FALSE,$project) {
        ...
        $ch = curl_init();
        // @see https://my.MicrosoftTeams.com/services/new/incoming-webhook
        // remove istance and token and add plugin_MicrosoftTeams_url config , see configurations with url above
        $url = sprintf('%s', trim($webhook));
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		$string = file_get_contents("http://127.0.0.1:9080/MessagesB2M.json");
        $payload = array(
